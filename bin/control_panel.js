...
var muteBtn;
var progressBar;
var statusCurrLine = 0;

function initialiseMediaPlayer() {
	// Get a handle to the player
	mediaPlayer = document.getElementById('recorder-controls');
...
function updateStatus(status){
    if(statusCurrLine > 10){
	statusCurrLine = 0;
	document.getElementById('status-panel').innerHTML = '<br>';
    }
    document.getElementById('status-panel').innerHTML += '<br>';
    document.getElementById('status-panel').innerHTML += status;
    statusCurrLine += 1;
}
function redoPlayer(){
    updateStatus('in redoPlayer');
}
